...
The front page has its own identity, deliberately unlike the shop pages: warm paper with a fine grain, Instrument Serif at one weight for every headline (the accent word italic, in oxblood), Instrument Sans for text, Geist Mono for the small labels, hairlines instead of boxes, ink buttons, one oxblood plate to close. It lives in `src/styles/home.css` (tokens under `@theme`: `paper`, `surface`, `print`, `rule`, `ox`; utilities `cta`, `label`, `plate`, `object`, `clipping`), which still imports the shop pages' stylesheet because the mockups on the page are the shop pages: anything that shows the product is wrapped in `.object`, which keeps the shop's type and colours inside a rounded, shadowed black frame while the page around it stays square-cornered and ruled. The three faces are self-hosted from `@fontsource` (latin subsets, about 96 KB, with metric-matched local fallbacks so the swap does not move a line) and, as TTFs in `src/og/fonts`, drawn into the share image.
