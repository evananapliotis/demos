...
| Variable | Unset | Set |
| --- | --- | --- |
| `CLIENT_SLUG` | a page per listing, `/` is the MyBarberSite offer | one page: that shop, at `/`. No `/<slug>` path at all |
...
A single-client build is a whole deploy, not a subset of the demo site, so `scripts/single-client.mjs` finishes it off in `astro:build:done`:

- **`dist/_redirects`** is rewritten. The shop's page is the root of the client's domain, so the paths it has on the demo site — `/<slug>` and the older `/demo/<slug>` — are 301s to `/`. Nothing renders a meta-refresh page; the demo site's own `public/_redirects` is replaced.
- **`dist/photos`, `dist/img` and `dist/brand` are pruned.** `public/` is copied whole, which here means every other shop's photos (over a gigabyte) and the whole MyBarberSite identity. Every file under those three that no built page reaches, directly or through a manifest it names, is deleted. For `ca1-barber-shop` that leaves 24 files and 2.6MB, down from 7,660 and 1.26GB; the build logs what it removed.

The pages are the shop's own website on the shop's own domain, not a demo of one, so a single-client build drops the `noindex,nofollow` meta and the footer's "Demo site for … Not the shop's official website" notice (`IS_CLIENT_SITE` in `src/config/build.ts`). `public/robots.txt` allows everything and is shipped as it is. The photo credits under the gallery stay on every build: they name the people who took the photos, which the page owes them whoever is serving it.

The link preview follows the front page: `/og.jpg` is the client's shop image on a single-client build (`src/pages/og.jpg.ts`), the MyBarberSite one otherwise, and `/<slug>/og.jpg` is not built.
...
scripts/single-client.mjs CLIENT_SLUG build: _redirects, prune unused public assets
